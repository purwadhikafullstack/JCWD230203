...
module.exports = {
    register: async(req, res) => {
        ...
        try {
            ...
            let findEmail = await tenant.findOne({where: {email}}, {transaction: t})

            if(findEmail){
                const httpResponse = new HttpResponse(res).error("Email already exist")
            ...
            const newTemplate = templateCompile({first_name, url:`http://localhost:3000/activation/${createTenant.dataValues.id}`, otp});
    ...
    activation: async(req, res) => {
      ...
      try {
          ...
          const template = await fs.readFile('./template/confirmation.html', 'utf-8');
          const templateCompile = await handlebars.compile(template);
          const newTemplate = templateCompile({first_name, url:`http://localhost:3000/activation/${tenants.dataValues.id}`, otp});
