...
function SearchRoom() {
  ...
  useEffect(() => {
    const getData = async() => {
      if(
       propertyName && priceMin && priceMax && sortOrder
      ){
        console.log("masuk")
        const res = await axios.get(`http://localhost:5000/properties/search-rooms?property_name=${propertyName}&price_min=${priceMin}&price_max=${priceMax}&sort_order=${sortOrder}`)
        setRoom(res.data.data);
      }else{
        console.log("masuk sini")
        const response = await axios.get(`http://localhost:5000/properties/search-date?check_in=${startDate}&check_out=${endDate}&city=${location}&page=${currentPage}`)
  ...
  return (
    <>
    {loc.pathname === `http://localhost:5000/properties/search-rooms?property_name=${propertyName}&price_min=${priceMin}&price_max=${priceMax}&sort_order=${sortOrder}` ? 
